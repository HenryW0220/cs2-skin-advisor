...
> 最后更新：2026-07-20（B2 剧本验证已跑完一版，见下）
...
3. **成交量数据薄，且已经卡住 B2/D1 的产出**：K线无量，只有 2026-06-22 起攒的小时在售数（1449 个操盘段落里只有 20 段有量覆盖）；挂单深度类数据源还没调研（PLAN A2 遗留）。B2 报告（见下）确认了"出货期指纹"和"吸货 vs 单纯横盘"这两个都需要量/深度数据才能验证，价格特征这条路走不通——量数据源调研从"顺路做"变成实打实的阻塞项。

## 四、下一步计划（按优先级）

1. **B2 操盘剧本验证第一版已产出**：`scripts/analyze-playbook-stages.mjs` + `REPORT-B2.md`。结论：低位横盘（占比最大）、洗盘/砸盘（深回撤>15%接急拉，指纹清晰）、主拉升（急拉簇，涨速12.69%/天中位数，指纹清晰）三段能验证；吸货期和单纯横盘价格形态上分不开、会员进场看不见、出货期没有独立于普通回调的指纹——三个都卡在缺量/挂单数据。**洗盘指纹可以先单独实现成 `lib/signals/` 里的一个特征上线**（"深回撤后接急拉"提示），不用等 C 阶段整体完成。
2. 量/挂单深度数据源调研（A2 遗留）**提级为阻塞项**：不做，C 阶段的吸货期特征和 D1 的逃顶信号都补不全。
