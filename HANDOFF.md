...
> 最后更新：2026-07-21 深夜（**生产环境从本机 Windows 迁到 Oracle Cloud（Always Free VM + Docker）**，本机 Windows 常驻采集器已停用，改成开发环境；公网访问加了 Basic Auth 最低限度保护；细节见"运行架构"一节和第五节第19/20条踩坑记录。当天早些时候还做了：PLAN.md 里 A2/D3/M2 三处过时的 Docker 描述改成实际的 Windows 原生方案——**这条现在又反过来过时了，因为晚上确实部署了 Docker，不用再改，历史记录留着**；C1/C2 用真正的全市场随机候选池复核过，结论比原先更不乐观，见第四节第4条和 REPORT-C1-C2.md；T+7 可行动标签第一版跑完，抓到并修复了评估方法的真实bug，"别追涨"和"洗盘回撤"是目前最可靠的复现信号；追涨风险信号上线；buy_price=0 资产离开库存不再落流水）
>
