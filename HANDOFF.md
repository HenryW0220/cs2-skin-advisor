...
7. **两个运维层面的待办（2026-07-21 发现，没在这次会话里处理）**：①云端 VM 的公网 IP 还不是 OCI 的 Reserved Public IP，HTTPS 证书绑定的 `sslip.io` 域名靠这个 IP 不变才有效，找时间去控制台转一下（免费，几分钟的事，见"运行架构"一节）。②`price_snapshots` 表一晚上从 65 万行涨到 88 万行，是因为观察池从 1 个饰品加到了 40 个，**每加一个新品会自动回填 90 天小时线**，40 个新品叠加起来涨得很快——目前有缓存顶着，页面不慢，但这张表会持续变大，是以后云端磁盘空间/备份体积的一个增长源，值得留意但暂时不是问题。
