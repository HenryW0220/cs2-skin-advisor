...
> 最后更新：2026-07-20（B2 剧本验证已跑完一版；洗盘信号上线；求购深度数据开始入库，见下）
...
1. **~212 条待审核异常等用户判断**（含新的嫌疑分/联动预警），集中在 buy_price=0 开箱品；千瓦收藏品 7 品、手套收藏品 4 品联动嫌疑最大。这是模型校准和 B2 重跑加密样本的前置条件。
...
3. 用户清完待审核 → 重跑 `scripts/analyze-manipulation-features.mjs` 校准嫌疑分阈值 + 验证 coMove；episode 数变多后重跑 `analyze-playbook-stages.mjs` 让三个簇的边界更稳
4. C 阶段预测模型（逻辑回归起步，precision@top-K + 提前量，须胜过随机和纯规则两个笨基准）
5. D3 产品化：PWA + Web Push 推送（最终目标是电脑+手机 App）
