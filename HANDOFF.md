...
> 最后更新：2026-07-21（PLAN.md 里 A2/D3/M2 三处过时的 Docker 描述改成实际的 Windows 原生方案；**C1/C2 用真正的全市场随机候选池复核过**，`scripts/backfill-candidate-pool.mjs` 全市场分层抽样500个补进 full 池，结论比原先更不乐观，见第四节第4条和 REPORT-C1-C2.md；Web Push 在独立浏览器里验证订阅/推送成功；**T+7 可行动标签第一版跑完**（`scripts/build-t7-actionable-baseline.mjs` + `REPORT-T7.md`），过程中抓到并修复了"同饰品同天多小时样本不去重导致平均收益虚高"的真实bug，修复后结论是"模型打赢基准"方向可信但幅度被极端值主导、"别追涨"和"洗盘回撤"两条是目前最可靠的复现信号；buy_price=0 资产离开库存不再落流水）
>
