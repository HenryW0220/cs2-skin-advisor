...
const KNOWN = [
  ...
  // ⚠️ 这一天最容易读错，2026-08-14 就读错过一次。当天**同时**发生两件事：
  // 观察池 41 → 282（watchlist 的一手记录，真实扩容 7 倍），以及 C5 提频到 10 分钟一轮。
  // 而 price_snapshots 的 distinct 饰品数**纹丝不动（325 → 325）**，行数翻倍。
  // 差点据此得出"没有扩容、只是写得更勤"的错误结论——实际是**那 241 个新增全部（241/241，
  // 100%）在加进观察池之前就已经在被采集了**（它们本来就在候选池里）。
  // 所以：扩容是真的，快照饰品数看不出来是因为采集集合本来就包含它们。
  // **这也解释了为什么扩容当天就能开出 79 笔仓位**——那批饰品已经有历史，信号立刻算得出来。
  // 教训：`price_snapshots` 的 distinct 饰品数是代理指标，**被追踪集合的一手记录是
  // `watchlist.created_at`**，用 scripts/report-tracking-scope.mjs 查。
  ["2026-07-26", "观察池 41 → 282（一手记录）+ C5 高频 tick 上线，行数近乎翻倍"],
