...
// AUC 中位数是 0.744~0.819。
// ⚠️ **这里原来还引了"求购深度那两个是 0.747"当依据，那个数已经作废**（踩坑 52）：
// 同一脚本在不同窗口/标注集下给出 0.583~0.747，是个 ±0.08 的高方差量，不能当推导依据。
// **0.60 这个下限本身不改**——它主要靠 0.744~0.819 那一组和"0.53 离能用差得远"这条
// 定性论证撑着，抽掉 0.747 不影响结论；但依据里不该再挂一个不稳的数。
// 所以取 0.60 作为
