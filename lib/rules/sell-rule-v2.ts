 ...
 *
 * ---- ⚠️ 上线至今的实盘验证覆盖率（2026-08-14 登记，讨论替换 v1 时必须先看这段）----
 * 回测覆盖了全部七档，**但实盘只有两条路真正跑过**，而且样本是个位数：
 *   · `SELL_STRONG`（>30%）：**1 次**，那一次的前向超额还是正的（+1.49%，即判错）；
 *   · `SELL`（15~30%）：**2 次**可评估，超额中位 -7.61%。
 * 也就是说"v2 已上线并行"给人的覆盖率印象，比"10 笔平仓"要小得多——
 * **七档里只有两档被实盘走到过，各自 1 笔和 2 笔。**
 *
 * ---- ⚠️ 洗盘否决这一条：它改不了任何决策，名字起错了 ----
 * 下面那个 `drawdown48h > WASHOUT_DRAWDOWN && return24h < WASHOUT_VETO_MAX_RETURN` 分支
 * **执行过（影子表里 4 条、3 个饰品），但它的 24h 涨幅全部落在 -15.6% ~ -0.2%**。
 * 这不是巧合，是**结构决定的**：它的生效条件是涨幅 < 5%，而它唯一可能拦下的两个动作
 * 要求涨幅 ≥ 15%——**两个区间不相交**。所以在它触发的地方，不走这个分支也一样是 HOLD，
 * 它改变的只有 `reason` 那句话，从来没有、也不可能否决掉一次卖出。
 * **"否决"这个叫法是错的，它实际是一条 HOLD 的理由文案**（对 LLM 生成的说法有价值，
 * 但不要把它当成一条经过验证的风控规则）。
 * 这个性质由 sell-rule-v2.test.ts 里"洗盘分支不改变 action"那条单测锁住。
 *
 * **为什么限制在低涨幅档是对的、不要去放宽**：2026-08-14 用 714 个饰品重跑，洗盘态在高涨幅
 * 档不但不保护、反而更差——15~20% 洗盘 -3.41% vs 非洗盘 -4.60% 尚可，但 20~30% 是
 * -7.14% vs -5.87%、>30% 是 -20.53% vs -18.58%，**都比不洗盘时更负**。想让这条"真的能否决
 * 一次卖出"，就得把它放宽到 ≥15% 档，而数据说那样会亏得更多。所以正确的结论是
 * **保留现状但改名**，不是去把它变得"有用"。
 * 讨论把 v2 接进 /positions 时，这一条要单独拿出来说，不能被 ">30% 档已回测验证" 顺带担保。
 *
 * ---- 刻意没有放进来的东西 ----
