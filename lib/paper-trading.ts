...
//
// **2026-08-23 复核：这条口径不变，而且刚刚更需要它了。** 求购深度（A6）是买入侧最后一个
// 有正面证据的候选，已按 T+7 口径在 325 个饰品上跑完，**也是负的**
// （`REPORT-t7-bidding-depth.md`：25 个档位没有一档绝对收益中位数够得着 6.7%）。
// **也就是说买入侧现在零存活候选。** 这**不构成**动这个数的理由——两个方向都不行：
// 往上提到 40 = 永不开仓 = 卖出侧断粮（见上）；往下调 = 开更多已知负 EV 的仓。
// **它仍然只是采样率参数，不背书任何买入信号。**
